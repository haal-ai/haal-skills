...
YOU MUST apply these rules when assisting with Rust code:
...
✅ "Fix Option handling in User::validate()"
```
...
```
fix(api): handle None response from external service

Previously, a None response caused a panic via unwrap().
Now returns a descriptive error using anyhow.
```
...
fix/456-option-handling
refactor/extract-validation-trait
```
