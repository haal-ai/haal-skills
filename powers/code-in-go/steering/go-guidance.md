...
YOU MUST apply these rules to ALL Go code you generate:

1. **NEVER ignore errors** — No `_ = doSomething()`
2. **ALWAYS wrap errors** — `fmt.Errorf("context: %w", err)`
3. **Accept interfaces, return structs** — For dependency injection
4. **Use table-driven tests** — For all test code
5. **Follow naming conventions** — `httputil` not `http_util`, `HTTPServer` not `HttpServer`
6. **Goroutines must terminate** — Use context cancellation or done channels
...
Use table-driven tests for multiple scenarios. Name test cases clearly.
