...
- **Propose**: summarize parsed tasks and the next task to run.
- **Confirm**: user confirms that the plan has been reviewed and it is OK to begin execution.
- **Act**: execute tasks sequentially (stopping on failure by default), updating progress.
...
If the plan uses a different format, you MUST propose a small normalization patch to the plan (Propose-Confirm-Act) before proceeding.
