...
✅ **Search-First Approach** - Checks `.olaf/data/product/` and `.olaf/data/practices/` first  
✅ **Fast Answers** - Returns results from existing docs (no analysis needed)  
...
- **Gap Check**: ✅ If build files exist | ⚠️ If no build config | ❌ If no code
...
Execution:
✓ Task 1: Parse → "how to build this project"
✓ Task 2: Direct question detected
✓ Task 3: Matched FAQ (confidence: 0.95)
✓ Task 6: Generated answer from build-and-run.md
...
### Example 2: No Question → Browse Mode
```
Input: olaf tell me
...
### Example 3: Novel Question → KB Search
```
Input: olaf tell me about database migrations

Execution:
✓ Task 1: Parse → "about database migrations"
✓ Task 2: Direct question (topic request)
✓ Task 3: No FAQ match (confidence: 0.42)
✓ Task 5: KB search found relevant content
✓ Task 6: Synthesized answer
...
├── tasks/
│   ├── parse-user-question.md
│   ├── check-direct-question.md
│   ├── match-faq.md
│   ├── present-options.md
│   ├── search-knowledge-base.md
│   └── generate-answer.md
└── kb/
    ├── faq-index.md            # 60 questions mapped to prompts
    └── documentation-artifacts.md  # 21 artifact specifications
```
